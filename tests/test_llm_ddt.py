...
def test_ai_response_quality(case,gemini_client, llm_as_judge):

    actual_response = gemini_client.ask(case['user_prompt'])

    result = llm_as_judge.evaluate(case["user_prompt"],actual_response, case['acceptance_criteria'])
    assert result['verdict'] == "PASS", f"Judge failed it:{result['reason']}"


def test_ai_first(gemini_client, llm_as_judge, time_tracker):
    actual_response = gemini_client.ask('What are the most useful features in Pytest? Answer in 2 sentence')
    evaluation = llm_as_judge.evaluate('What are the most useful features in Pytest? Answer in 2 sentence', actual_response,
